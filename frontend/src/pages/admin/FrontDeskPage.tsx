...
export default function FrontDeskPage() {
  ...
  const { data: todayData, isLoading: loadingToday, refetch } = useTodayFrontDesk()
  const { data: rangeData, isLoading: loadingRange } = useFrontDeskRange(start, end)
  const { data: analysisData, isLoading: loadingAnalysis } = useFrontDeskAnalysis(currentMonth)
  const { data: historyData, isLoading: loadingHistory } = useFrontDeskHistory(7)
  ...
  function badgeVariant(count: number): 'green' | 'yellow' | 'orange' | 'red' {
    ...
    if (count >= TOTAL / 2) return 'yellow'
          ...
          <SectionCard>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                <h2 className="font-display text-base font-semibold">Front Desk Daily Checklist Status</h2>
                {todayData && (
                  <Badge variant={badgeVariant(todayData.completed_count)}>
                    {todayData.completed_count}/{TOTAL} completed
                  </Badge>
                )}
              </div>
              <button

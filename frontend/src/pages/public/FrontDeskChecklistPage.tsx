...
import { FRONT_DESK_TASKS } from '../../lib/constants'
import { useTodayFrontDesk, useSubmitFrontDeskChecklist } from '../../hooks/useFrontDesk'
import ChecklistItem from '../../components/checklist/ChecklistItem'
import SubmitButton from '../../components/checklist/SubmitButton'
import LoadingSpinner from '../../components/ui/LoadingSpinner'

const TOTAL = FRONT_DESK_TASKS.length

export default function FrontDeskChecklistPage() {
  const { data: todayData, isLoading } = useTodayFrontDesk()
  const submitMutation = useSubmitFrontDeskChecklist()
  ...
  if (todayData && !initialized) {
    ...
    setInitialized(true)
    if (todayData.completed_count === TOTAL) setAllComplete(true)
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (checked.size === TOTAL) setAllComplete(true)
    } catch (e: unknown) {
  ...
  // Show the "X of N saved" note on page load when there's a partial save from a previous session
  const showSavedNote = initialized && savedCount > 0 && savedCount < TOTAL && !lastSavedAt
      ...
      <div className="min-h-screen bg-[#F8F7F5] flex flex-col items-center justify-center px-4">
        <div className="bg-white rounded-card shadow-sm border border-gray-100 p-10 max-w-sm w-full text-center">
          ...
          <p className="text-gray-500 text-sm mt-3">
            All {TOTAL} tasks have been checked off. Great work — see you tomorrow!
          </p>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-[#F8F7F5] flex flex-col items-center px-4 py-10">
      <div className="w-full max-w-[500px]">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs uppercase tracking-[0.2em] text-gray-400 mt-1">Front Desk Daily Checklist</p>
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-card shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              ...
              {showSavedNote && (
                <p className="text-xs text-orange font-semibold mt-1">
                  {savedCount} of {TOTAL} tasks saved — complete the rest and save again
                </p>
              )}
            </div>

            <div className="divide-y divide-gray-100">
              {FRONT_DESK_TASKS.map((task) => (
                <ChecklistItem
                  key={task.id}
                  taskId={task.id}
                  label={task.label}
            ...
            <div className="px-5 py-4 border-t border-gray-100">
              <p className="text-xs text-gray-400 mb-4">{checked.size} of {TOTAL} tasks completed</p>

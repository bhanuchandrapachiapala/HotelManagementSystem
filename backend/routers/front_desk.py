...
from models.task import (
    FrontDeskSubmitChecklistRequest,
...
TOTAL_TASKS = len(FRONT_DESK_VALID_TASK_IDS)
...
def get_today_tasks():
    ...
    db = get_supabase()
    result = db.table("front_desk_task_completions").select("*").eq("date", today).execute()
    rows = result.data or []
...
def get_tasks_for_date(date_str: str):
    db = get_supabase()
    result = db.table("front_desk_task_completions").select("*").eq("date", date_str).execute()
    rows = result.data or []
    if not rows:
        return {
            "date": date_str,
...
def get_tasks_range(
    ...
    db = get_supabase()
    result = (
        db.table("front_desk_task_completions")
        .select("*")
        .gte("date", start_date)
        .lte("date", end_date)
        .execute()
    ...
    fully_completed = sum(1 for d in days if d["completed_count"] == TOTAL_TASKS)
    ...
    if all_counts:
        overall_rate = round(sum(all_counts) / (len(all_counts) * TOTAL_TASKS) * 100, 1)
    else:
        overall_rate = 0.0
...
def get_task_analysis(month: str = Query(default=None)):
    ...
    by_task: dict[str, set] = {tid: set() for tid in FRONT_DESK_VALID_TASK_IDS}
    for row in rows:
        if row["task_id"] in by_task:
            by_task[row["task_id"]].add(row["date"])

    tasks = []
    for tid in FRONT_DESK_VALID_TASK_IDS:
        ...
        tasks.append(
            {
                "task_id": tid,
                "label": FRONT_DESK_TASK_LABELS[tid],
...
def submit_checklist(body: FrontDeskSubmitChecklistRequest):
    print(f"[frontdesk submit] received date={body.date}, server_today={date.today()}")
    db = get_supabase()
    db.table("front_desk_task_completions").delete().eq("date", body.date).execute()
    ...
    db.table("front_desk_task_completions").insert(rows).execute()
